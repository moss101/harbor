...
## Session 26 additions (DOCX preview dispatch + Work Canvas rendering)

Completion-audit finding fixed: `artifact.preview` misrouted DOCX files
into the workbook branch (guaranteed error). Now:
- FFI dispatch by content types: wordprocessingml -> DocxPreview
  (paragraphs + styles + preserved-parts report), presentationml -> deck,
  %PDF -> pdf pages, else workbook
- harbor_render::DocxPreview (paragraphs/styles IR) + Work Canvas
  rendering branches for docx/pdf previews
- Dart FFI e2e test: real structured.docx fixture (generated by
  harbor_artifacts example) previews through the boundary with the
  "Harbor Plan" heading verified
- 19/19 app tests, 179 Rust tests, dossier PASS — unchanged
