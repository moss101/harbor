...
fn reenrolling_device_downloads_live_tails_and_uploads_on_new_epoch() {
    ...
    let bundle = seal_bundle(
        ...
        t91,
        t91,
        &tails.iter().cloned().collect::<Vec<_>>(),
    )
    .unwrap();
    ...
    // tails.
    let opened = open_bundle(&group, &bundle, &VerifyingKey::from(&SigningKey::from_bytes(&issuer_seed))).unwrap();
    assert!(opened.is_empty(), "epoch-1 tails must not ride an epoch-2 bundle");
    ...
    .unwrap();
    let opened = open_bundle(&group, &bundle2, &VerifyingKey::from(&SigningKey::from_bytes(&issuer_seed))).unwrap();
...
fn stale_bundle_epoch_is_rejected_before_decryption() {
    ...
    assert!(matches!(
        open_bundle(&group, &bundle, &VerifyingKey::from(&SigningKey::from_bytes(&issuer_seed))),
