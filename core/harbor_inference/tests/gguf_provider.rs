...
/// Exact token counts for each shipped schema's worst-case output.
///
/// `#[ignore]` — needs the 1.1 GB catalog model. Run by hand:
///   cargo test -p harbor_inference --features gguf-backend --test gguf_provider \
///     -- --ignored worst_case_output_against_budget --nocapture
///
/// STATUS records that most `model.structured` nodes declare schemas
/// larger than their own `max_tokens`. That table was built on a
/// chars/4 estimate. For `formula-audit` (49x) and `meeting-notes` (17x)
/// the approximation cannot change the verdict, but `team-update` sat at
/// 1.13x — inside the error bar, which means the published claim about
/// it was an estimate wearing the clothes of a measurement. This counts
/// tokens with the tokenizer the model actually uses.
#[test]
#[ignore]
fn worst_case_output_against_budget_in_real_tokens() {
    let weights = repo_root().join("fixtures/models/qwen2.5-1.5b-instruct-q4_k_m.gguf");
    if !weights.exists() {
        eprintln!("skipped: {} not present", weights.display());
        return;
    ...
    // Build the worst cases here, from the shipped graphs. An earlier
    // version read them from a scratch file I had generated by hand,
    // which would have made this test pass on my machine and nowhere
    // else — the same "it works where I ran it" mistake the rest of
    // this file exists to catch.
    let mut cases: Vec<(String, String, u64, String)> = Vec::new();
    let mut paths: Vec<_> = std::fs::read_dir(repo_root().join("core/harbor_core/src/graphs"))
        .unwrap()
    ...
    for path in paths {
        ...
        for n in g["nodes"].as_array().unwrap() {
            let Some(sc) = n.get("output_schema") else {
                continue;
            };
            let budget = n.get("max_tokens").and_then(|v| v.as_u64()).unwrap_or(1024);
            cases.push((
                stem.clone(),
    ...
    println!(
        "\n{:<26} {:<12} {:>8} {:>10} {:>8}",
        "graph", "node", "budget", "worst_tok", "ratio"
    );
    for (graph, node, budget, text) in &cases {
        let toks = model
        ...
        println!(
            ...
            budget,
            toks,
            toks as f64 / *budget as f64,
            if toks > *budget { "  OVER" } else { "" }
        );
    }
}

/// The largest JSON text a schema permits, as an actual string so it can
/// be tokenised rather than estimated. Strings are filled to maxLength
/// and arrays to maxItems: no real answer looks like this, which is the
/// point — it is the ceiling the grammar will let a model walk to.
fn worst_case_json(sc: &serde_json::Value) -> String {
    let t = match &sc["type"] {
    ...
    match t.as_str() {
        "object" => {
            let props = sc["properties"].as_object().cloned().unwrap_or_default();
            let body: Vec<String> = props
                .iter()
                .map(|(k, v)| format!("\"{k}\":{}", worst_case_json(v)))
                .collect();
            format!("{{{}}}", body.join(","))
        }
        "array" => {
            let n = sc["maxItems"].as_u64().unwrap_or(50) as usize;
            format!("[{}]", vec![worst_case_json(&sc["items"]); n].join(","))
        }
        "string" => {
            if let Some(e) = sc["enum"].as_array() {
                let longest = e
                    .iter()
            ...
            let n = sc["maxLength"].as_u64().unwrap_or(200) as usize;
            format!("\"{}\"", "w".repeat(n))
        }
        "integer" | "number" => "123456789012".into(),
        "boolean" => "false".into(),
        _ => "null".into(),
    }
}
