...
def run_regressions():
    ...
    with tempfile.TemporaryDirectory(prefix='harbor-ledger-test-') as tmp:
        ...
        shutil.copytree(ROOT,root,ignore=shutil.ignore_patterns('assets','reviews','outputs','*.docx','__pycache__','target','.git','.github','build','.dart_tool','.gradle','ephemeral','.idea','Pods','models','models-store','evidence','*.gguf','node_modules'))  # machine-local model weights and evidence are not package inputs
